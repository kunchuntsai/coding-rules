...
// Or use factory pattern with custom deleter
// module_a.h
using BufferDeleter = void(*)(char*);  // Function pointer type for deleter
...
// module_a.cpp
BufferPtr createBuffer(size_t size) {
    // Custom deleter: a lambda that will run in module_a's context
    auto deleter = [](char* p) { delete[] p; };

    return BufferPtr(new char[size], deleter);
}

// Or use factory pattern with smart pointer and deleter
// module_a.h
